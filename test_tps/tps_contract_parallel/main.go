...
func deployMultipleContracts(client *ethclient.Client, accounts []AccountInfo, numContracts int, chainID int64, bytecode []byte) ([]*common.Address, error) {
	if numContracts > len(accounts) {
		return nil, fmt.Errorf("không đủ tài khoản để deploy %d contracts", numContracts)
	}
	
	contractAddrs := make([]*common.Address, numContracts)
	...
	for i := 0; i < numContracts; i++ {
		...
		go func(idx int) {
			...
			acc := accounts[idx]
			pkBytes, _ := hex.DecodeString(acc.PrivateKey)
			pk, _ := crypto.ToECDSA(pkBytes)
			from := crypto.PubkeyToAddress(pk.PublicKey)

			addr, err := deployContract(client, pk, chainID, from, bytecode)
			mu.Lock()
			if err != nil {
				if firstErr == nil {
					firstErr = err
...
func main() {
	...
	// Deploy N contracts concurrently
	ethClient, err := ethclient.Dial(rpcPool[0].Endpoint)
	if err != nil {
		log.Fatalf("❌ Lỗi kết nối RPC: %v", err)
	}

	fmt.Printf("  🚀 Deploying %d contracts via RPC...\n", numContracts)
	contractAddrs, err := deployMultipleContracts(ethClient, accounts, numContracts, int64(chainId), bytecode)
	if err != nil {
		log.Fatalf("❌ Deploy %d contracts thất bại: %v", numContracts, err)
	}
	fmt.Printf("  📌 Đã deploy xong %d contracts. Contract đầu tiên: %s\n\n", numContracts, contractAddrs[0].Hex())

	// Force nonce refresh for accounts that deployed contracts
	for i := 0; i < numContracts; i++ {
		addr := accounts[i].Address
		nonceMap[addr] = nonceMap[addr] + 1
	}
	...
	for _, acc := range toSend {
		...
		// Phân phối đều giao dịch cho N contracts đã deploy bằng thuật toán round-robin
		targetContract = *contractAddrs[len(allTxs)%numContracts]
	...
	for round := 1; round <= numRounds; round++ {
		...
		if round > 1 {
			...
			for idx, acc := range toSend {
				...
				// Phân phối đều giao dịch cho N contracts đã deploy bằng thuật toán round-robin
				targetContract = *contractAddrs[idx%numContracts]
