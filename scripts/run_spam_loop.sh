...
echo "=================================================="
echo "🔄 BẮT ĐẦU VÒNG LẶP TEST - DEBUG - FIX SPAM"
echo "=================================================="

# Dọn dẹp các tiến trình cũ trước khi chạy
pkill -f "ci_monitor.py" || true
sleep 1

# Run ci_monitor.sh với type spam, --batch 300, và --no-listen
./ci_monitor.sh --type spam --batch 300 --no-listen --clean-logs

# Chờ 5 giây để Python monitor khởi tạo xong
sleep 5

# Tìm PID của ci_monitor.py
MONITOR_PID=$(pgrep -f "ci_monitor.py" | grep -v "$$" | head -n 1)

if [ -z "$MONITOR_PID" ]; then
    echo "❌ Không tìm thấy tiến trình ci_monitor.py!"
...
echo "📄 Thư mục Logs: $SCRIPT_DIR/auto_test_logs"

# Lấy log file mới nhất để theo dõi
LATEST_LOG=$(ls -t "$SCRIPT_DIR/auto_test_logs"/*.log 2>/dev/null | head -n 1)
...
while kill -0 "$MONITOR_PID" 2>/dev/null; do
    ...
    if [ $((CHECK_COUNT % 4)) -eq 0 ]; then
        ...
        LATEST_LOG_CURRENT=$(ls -t "$SCRIPT_DIR/auto_test_logs"/*.log 2>/dev/null | head -n 1)
...
echo "🛑 Tiến trình ci_monitor.py đã dừng!"
