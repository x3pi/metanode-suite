...
echo "=================================================="
echo "🔄 BẮT ĐẦU VÒNG LẶP TEST - DEBUG - FIX XAPIAN"
echo "=================================================="

# Dọn dẹp các tiến trình cũ trước khi chạy
pkill -f "ci_spam_xapian_monitor.py" || true
sleep 1
...
# Tìm PID của ci_spam_xapian_monitor.py
MONITOR_PID=$(pgrep -f "ci_spam_xapian_monitor.py" | grep -v "$$" | head -n 1)

if [ -z "$MONITOR_PID" ]; then
    echo "❌ Không tìm thấy tiến trình ci_spam_xapian_monitor.py!"
...
echo "📄 Thư mục Logs: $SCRIPT_DIR/spam_xapian_logs"

# Lấy log file mới nhất để theo dõi
LATEST_LOG=$(ls -t "$SCRIPT_DIR/spam_xapian_logs"/*.log 2>/dev/null | head -n 1)
...
while kill -0 "$MONITOR_PID" 2>/dev/null; do
    ...
    # Mỗi 1 phút in ra dòng status hoặc 10 dòng log mới nhất để giữ terminal hoạt động
    if [ $((CHECK_COUNT % 4)) -eq 0 ]; then
        echo "⏰ [$(date '+%H:%M:%S')] Monitor vẫn đang chạy..."
        LATEST_LOG_CURRENT=$(ls -t "$SCRIPT_DIR/spam_xapian_logs"/*.log 2>/dev/null | head -n 1)
...
echo "🛑 Tiến trình ci_spam_xapian_monitor.py đã dừng!"
