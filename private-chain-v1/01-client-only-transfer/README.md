...
```bash
cd /home/abc/nhat/con-chain-v2/metanode-suite/private-chain-v1/01-client-only-transfer
go run .
```

---

## 📁 Smart Contracts Kèm Theo (`contracts/`)
- [`contracts/IGateway.sol`](contracts/IGateway.sol): Interface hợp đồng Gateway precompile (`0x...1002`) với hàm `outbound(...)`.
- [`contracts/TestCounter.sol`](contracts/TestCounter.sol): Mã nguồn Solidity của contract đếm số được deploy trên Chain Đích.
