...
var (
	...
	fileLoggerCurrentDate  string
	fileLoggerRotationStop chan struct{}
...
func Error(message interface{}, a ...interface{}) {
	...
	// Write to configured outputs (console/file) using color buffer
	colored := getLogBuffer(Red, "ERROR", message, a)
	logger.writeToOutputs(colored)
	logger.writeToError(colored)
}
...
// EnableDailyFileLog bật ghi log vào file theo ngày (dựa vào loggerfile global dir).
...
func EnableDailyFileLog(fileName string) (*loggerfile.FileLogger, error) {
	...
	fileLoggerCurrentDate = time.Now().Format("2006-01-02")
	attachFileLoggerOutputLocked()
	startFileLoggerRotationLocked()
...
func startFileLoggerRotationLocked() {
	stopFileLoggerRotationLocked()
	fileLoggerRotationStop = make(chan struct{})
	stopChan := fileLoggerRotationStop
	fileName := fileLoggerFileName
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				fileLoggerMu.Lock()
				if fileLoggerInstance == nil || fileLoggerFileName != fileName {
					fileLoggerMu.Unlock()
					return
				}
				if nowDate != fileLoggerCurrentDate {
					rotateFileLoggerLocked(nowDate)
				}
				fileLoggerMu.Unlock()
			case <-stopChan:
				return
...
func stopFileLoggerRotationLocked() {
	if fileLoggerRotationStop != nil {
		close(fileLoggerRotationStop)
		fileLoggerRotationStop = nil
	}
}

func rotateFileLoggerLocked(newDate string) {
	newLogger, err := loggerfile.NewFileLogger(fileLoggerFileName)
	if err != nil {
		fmt.Printf("failed to rotate daily log file: %v\n", err)
		return
	}
	...
	fmt.Printf("log rotated to date %s (file %s)\n", newDate, fileLoggerFileName)
}
