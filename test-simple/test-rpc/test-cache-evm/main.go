...
func main() {
	...
	// BẮN VÀ CHỜ TỪNG GIAO DỊCH
	fmt.Println("Đang gửi Tx 1 (deposit 1 coin) để Cache Balance vào C++ EVM...")
	if err := client.SendTransaction(context.Background(), signedTx1); err != nil {
		fmt.Printf("❌ Lỗi khi gửi Tx 1: %v\n", err)
		return
	}
	rcpt1, err := waitReceipt(client, signedTx1.Hash())
	if err != nil || rcpt1.Status == 0 {
		fmt.Printf("❌ Tx 1 thất bại hoặc timeout: %v\n", err)
		return
	}

	fmt.Println("Đang gửi Tx 2 (Native Transfer) để gửi trả 8 coin cho Master...")
	if err := client.SendTransaction(context.Background(), signedTx2); err != nil {
		fmt.Printf("❌ Lỗi khi gửi Tx 2: %v\n", err)
		return
	}
	rcpt2, err := waitReceipt(client, signedTx2.Hash())
	if err != nil || rcpt2.Status == 0 {
		fmt.Printf("❌ Tx 2 thất bại hoặc timeout: %v\n", err)
		return
	}

	fmt.Println("Đang gửi Tx 3 (step3) để đọc lại Balance của ví gửi vào EVM...")
	if err := client.SendTransaction(context.Background(), signedTx3); err != nil {
		fmt.Printf("❌ Lỗi khi gửi Tx 3: %v\n", err)
		return
	}
	rcpt3, err := waitReceipt(client, signedTx3.Hash())
	if err != nil || rcpt3.Status == 0 {
