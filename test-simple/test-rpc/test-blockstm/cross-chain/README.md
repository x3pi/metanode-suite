...
| **[`04-cross-chain-caro-game/`](./04-cross-chain-caro-game)** | **Demo Game Caro Xuyên Chuỗi (EVM GMP)**: Người chơi X ở Chain 101 đấu với Người chơi O ở Chain 102 qua Public Chain 991 với đặt cược và đồ họa bàn cờ màu sắc. | `cd 04-cross-chain-caro-game && go run .` |
| **[`05-relayer-network/`](./05-relayer-network)** | **Standalone Relayer Network Daemon (24/7 Service)**: Tiến trình Relayer chạy ngầm độc lập tự động phát hiện, gom chữ ký BFT Quorum và nộp giao dịch liên chuỗi. | `cd 05-relayer-network && go run relayer_daemon.go` |
