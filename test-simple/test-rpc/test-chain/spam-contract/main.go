...
func main() {
	...
	// Deployer key: ưu tiên ví chính trong config.json (đảm bảo luôn có số dư lớn)
	deployerHex := cfg.PrivateKey
	if deployerHex == "" && len(cfg.PrivateKeys) > 0 {
		deployerHex = cfg.PrivateKeys[0]
	}
	if deployerHex == "" {
		deployerHex = testKeys[0]
	}
	pkDeployer, err := crypto.HexToECDSA(deployerHex)
	if err != nil {
		log.Fatalf("❌ Lỗi parse deployer key: %v", err)
	}
	fromDeployer := crypto.PubkeyToAddress(*pkDeployer.Public().(*ecdsa.PublicKey))

	log.Printf("🚀 Deploying contract with Deployer Account (%s)...", fromDeployer.Hex())
	contractAddr, err := deployContract(client, pkDeployer, cfg.ChainID, fromDeployer, bytecode)
	if err != nil {
	...
	if *useXapian {
		...
		initHash, err := sendInitializeDoc(client, pkDeployer, cfg.ChainID, fromDeployer, contractAddr, parsedABI)
		if err != nil {
	...
	for r := 1; r <= *rounds; r++ {
		...
		for {
				...
				if *useXapian {
					if *useParallel {
						...
						if err != nil {
							...
							nodePassed = (roundActual == expectedVal)
						}
					}
				} else {
					if *useParallel {
			...
			var nodeStatusSummary []string
			for i, node := range checkNodes {
				statusStr := fmt.Sprintf("%s[B:%d, Val:%d]", node.Name, nodeBlocks[i], nodeResults[i])
				if nodeBlocks[i] < maxBlock || nodeResults[i] != expectedVal {
					statusStr += "❌"
				} else {
					statusStr += "✅"
				}
				nodeStatusSummary = append(nodeStatusSummary, statusStr)
			}

			if firstCheck {
				log.Printf("⏳ Phát hiện có node chưa khớp state hoặc block (MaxBlock: %d | Kỳ vọng %s: %d). Chi tiết: %s. Bắt đầu chờ tối đa 4 phút...", maxBlock, valName, expectedVal, strings.Join(nodeStatusSummary, ", "))
				firstCheck = false
			} else {
				log.Printf("   [⏳ Sync Waiting %v/4m] MaxBlock: %d | Kỳ vọng %s: %d | %s (Đang thử lại sau 5s...)", time.Since(syncStart).Round(time.Second), maxBlock, valName, expectedVal, strings.Join(nodeStatusSummary, ", "))
			}
