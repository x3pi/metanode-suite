...
func RunTest(configPath string) error {
	...
	if len(cfg.PrivateKeys) < 10 {
		return fmt.Errorf("cần ít nhất 10 private keys cho bài test này")
	}
	...
	fmt.Printf("📌 ParallelXapian deployed at: %s\n\n", contractAddr.Hex())

	var wg sync.WaitGroup
	var errs []error
	var errsMu sync.Mutex

	fmt.Printf("🔥 Gửi 9 tx AddDocument và 1 tx SearchDocument đồng thời...\n")
	start := time.Now()
	...
	for i, pkStr := range testKeys {
		...
		go func(idx int, pKeyHex string) {
			...
			if idx < 9 {
				// 9 giao dịch addDocument
				text := fmt.Sprintf("hello world from transaction number %d", idx)
				hash, err = sendTx(client, pk, cfg.ChainID, from, contractAddr, contractABI, "addDocument", text)
			} else {
				// 1 giao dịch searchDocument
				hash, err = sendTx(client, pk, cfg.ChainID, from, contractAddr, contractABI, "searchDocument", "hello")
			}
	...
	if err != nil {
		return fmt.Errorf("lỗi gửi search: %w", err)
	}
	searchReceipt, err := waitReceipt(client, searchHash)
	if err != nil || searchReceipt.Status != 1 {
		return fmt.Errorf("searchDocument sau commit bị lỗi")
	}
	...
	return fmt.Errorf("test failed: kỳ vọng totalFound >= 9 nhưng nhận %d", totalFound)
}
