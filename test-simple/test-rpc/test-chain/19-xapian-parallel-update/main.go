...
func RunTestWithOptions(opts TestOptions) error {
	...
	if err != nil {
		return fmt.Errorf("❌ InitializeDoc failed: %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("❌ Lỗi lấy Header: %v", err)
	}
	...
	for r := 1; r <= opts.Rounds; r++ {
		...
		if err != nil {
			return fmt.Errorf("❌ Lỗi getUserDataFromDB(): %v", err)
		}
...
func main() {
	rounds := flag.Int("rounds", 1, "Số vòng lặp gửi tx")
	numKeys := flag.Int("keys", 10, "Số lượng private key sử dụng")
	keysFile := flag.String("keys-file", "", "Đường dẫn file chứa private keys")
	waitMethod := flag.String("wait-method", "block", "Phương pháp chờ tx")
	flag.Parse()
