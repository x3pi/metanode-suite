...
func RunTest(configPath string) error {
	...
	if err != nil {
		return fmt.Errorf("❌ Lỗi decode bytecode: %v", err)
	}

	if len(cfg.PrivateKeys) < 10 {
		return fmt.Errorf("❌ Cần ít nhất 10 private key cho bài test này (hiện có %d)", len(cfg.PrivateKeys))
	}

	pk0Str := cfg.PrivateKeys[0]
	pk0, err := crypto.HexToECDSA(pk0Str)
	if err != nil {
	...
	nonce0, err := client.PendingNonceAt(context.Background(), from0)
	if err != nil {
		return fmt.Errorf("❌ Lỗi lấy pending nonce 0: %v", err)
	}
	setupHash, err := sendTxWithNonce(client, pk0, cfg.ChainID, from0, contractAddr, contractABI, nonce0, "setupDocument", "INITIAL_DATA")
	if err != nil {
		return fmt.Errorf("❌ Setup thất bại: %v", err)
	}
	if _, err := waitReceipt(client, setupHash); err != nil {
		return fmt.Errorf("❌ Lỗi chờ setup receipt: %v", err)
	}
	...
	if successCount > 0 {
		...
		return fmt.Errorf("⚠️ KẾT LUẬN: Không đọc được dữ liệu. Các giao dịch Read đều trả về rỗng")
	}
	return nil
}
