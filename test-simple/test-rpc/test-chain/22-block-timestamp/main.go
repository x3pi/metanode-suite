...
func RunTestWithOptions(opts TestOptions) error {
	...
	if err != nil {
		return fmt.Errorf("❌ Lỗi gửi giao dịch saveAll(): %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("❌ Lỗi lấy receipt: %v", err)
	}

	blockInfo, err := client.BlockByNumber(context.Background(), receipt.BlockNumber)
	if err != nil {
		return fmt.Errorf("❌ Lỗi lấy thông tin block bằng RPC: %v", err)
	}
	...
	return fmt.Errorf("⚠️ TEST FAILED: Các giá trị BlockContext truyền cho EVM bị lỗi")
}
